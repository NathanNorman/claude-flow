...
class ProductionValidator {
  ...
  private async validateBuildSystem(): Promise<void> {
    ...
    try {
      const lockExists =
        (await fs.pathExists('package-lock.json')) || (await fs.pathExists('pnpm-lock.yaml'));
      if (lockExists) {
    ...
    try {
      ...
      if (testFiles.length >= 5) {
        this.addResult(
          'Production Tests',
          'pass',
          `Found ${testFiles.length} production test files`,
        );
      } else if (testFiles.length > 0) {
        this.addResult(
          'Production Tests',
          'warning',
          `Only ${testFiles.length} production test files found`,
        );
      } else {
  ...
  private async validateMockImplementations(): Promise<void> {
    ...
    if (mockCount === 0) {
      ...
      this.addResult(
        'Mock Implementations',
        'warning',
        `${mockCount} files with mock implementations found`,
        mockFiles,
      );
    } else {
      this.addResult(
  ...
  private async validateSecurityConfiguration(): Promise<void> {
    ...
    if (presentVars.length > 0) {
      this.addResult(
        ...
        'pass',
        `${presentVars.length} security environment variables configured`,
      );
    } else {
  ...
  private async validateDeploymentReadiness(): Promise<void> {
    ...
    if (healthCheckFiles.length > 0) {
      this.addResult(
        'Health Checks',
        'pass',
        `${healthCheckFiles.length} health check files found`,
      );
    } else {
