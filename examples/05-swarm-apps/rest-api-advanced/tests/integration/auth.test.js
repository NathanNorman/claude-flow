...
describe('Auth Endpoints', () => {
  ...
  describe('POST /api/auth/register', () => {
    it('should register a new user successfully', async () => {
      ...
      const response = await request(server).post('/api/auth/register').send(userData).expect(201);
    ...
    it('should validate required fields', async () => {
      ...
      expect(response.body.errors).toContainEqual(expect.objectContaining({ field: 'password' }));
  ...
  describe('POST /api/auth/refresh', () => {
    ...
    it('should not refresh with used token', async () => {
      // Mark token as used
      await Token.updateOne({ token: refreshToken }, { used: true, usedAt: new Date() });
  ...
  describe('POST /api/auth/reset-password', () => {
    ...
    it('should not reset with expired token', async () => {
      // Expire the token
      await Token.updateOne({ token: resetToken }, { expiresAt: new Date(Date.now() - 1000) });
