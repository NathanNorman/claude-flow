...
describe('AuthService', () => {
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should throw error for inactive user', async () => {
      ...
      await expect(authService.refreshAccessToken(refreshToken)).rejects.toThrow(
        new ApiError('Account has been deactivated', 401),
      );
    });
  });
  ...
  describe('blacklistToken', () => {
    it('should blacklist token in Redis', async () => {
      ...
      expect(mockRedis.setex).toHaveBeenCalledWith(`blacklist_${token}`, expect.any(Number), '1');
    });
    ...
    it('should use provided TTL', async () => {
      ...
      expect(mockRedis.setex).toHaveBeenCalledWith(`blacklist_${token}`, ttl, '1');
    });
