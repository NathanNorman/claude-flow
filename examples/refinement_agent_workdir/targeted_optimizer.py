...
class TargetedOptimizer:
    ...
    def optimize_hyperparameters_grid(
        ...
        logger.info(
            f"Grid Search complete. Best score: {grid_search.best_score_:.4f} "
    ...
    def optimize_hyperparameters_bayesian(
        ...
        self.optimization_history.append(result)
        logger.info(
            f"Bayesian optimization complete. Best score: {bayes_search.best_score_:.4f} "
            f"(+{improvement:.2%} improvement)"
        )
    ...
    def optimize_with_optuna(
        ...
        self.optimization_history.append(result)
        logger.info(
            f"Optuna optimization complete. Best score: {best_score:.4f} "
            f"(+{improvement:.2%} improvement)"
        )
    ...
    def optimize_feature_engineering(
        self, X: np.ndarray, y: np.ndarray, base_model: BaseEstimator
    ) -> OptimizationResult:
        """
        ...
        def objective(trial):
            ...
            poly_degree = (
                trial.suggest_int("poly_degree", 2, 3) if use_polynomial else 2
            )
            ...
            feature_selection_k = trial.suggest_int(
                "feature_selection_k", int(X.shape[1] * 0.5), X.shape[1]
            )
            ...
            selector = SelectKBest(
                score_func=(
                    f_classif if self.evaluation_metric == "accuracy" else f_regression
                ),
        ...
        return self.optimize_with_optuna(
            objective_func=objective, component_name="feature_engineering", n_trials=50
        )
    ...
    def _extract_best_configs(self) -> Dict[str, Any]:
        ...
        for result in self.optimization_history:
            ...
            if (
                component_base not in best_configs
                or result.best_score > best_configs[component_base]["score"]
            ):
    ...
    def export_results(self, filepath: str):
        ...
        export_data = {
            ...
            "summary": {
                ...
                "best_achieved_score": max(
                    r.best_score for r in self.optimization_history
                ),
...
class AdaptiveOptimizer:
    ...
    def optimize_adaptive(
        ...
        if total_combinations < 100 and n_continuous == 0:
            ...
            logger.info(
                f"Using Grid Search for {component_name} "
            ...
            logger.info(
                f"Using Bayesian optimization for {component_name} "
            ...
            logger.info(
                f"Using Randomized Search for {component_name} "
