...
def run_refinement_workflow():
    ...
    baseline_pipeline = {
        ...
        "feature_engineer": FeatureEngineer(
            polynomial_features=False, interaction_features=False, log_transform=False
        ),
    ...
    ablation_analyzer = AblationAnalyzer(
        baseline_pipeline=baseline_pipeline, evaluation_metric="accuracy", cv_folds=5
    )
    ...
    ablation_results = ablation_analyzer.run_full_ablation(
        components_to_test=components_to_test, X=X_train, y=y_train
    )
    ...
    logger.info(
        f"Baseline performance: {ablation_results['baseline_performance']['accuracy_mean']:.4f}"
    )
    logger.info("\nComponent Rankings by Impact:")
    for ranking in ablation_results["component_rankings"]:
        logger.info(
            f"  {ranking['rank']}. {ranking['component']}: "
    ...
    iterative_results = targeted_optimizer.iterative_refinement(
        components=refinement_components, X=X_train, y=y_train, max_iterations=3
    )
    ...
    for component, config in best_configs.items():
        logger.info(
            f"  {component}: score={config['score']:.4f}, method={config['method']}"
        )
    ...
    final_summary = {
        ...
        "optimization_time": sum(
            r.optimization_time for r in targeted_optimizer.optimization_history
        ),
    ...
    logger.info(
        f"Total improvement achieved: {iterative_results['total_improvement']:.2%}"
    )
...
def demonstrate_adaptive_optimization():
    ...
    targeted_opt = TargetedOptimizer(
        baseline_score=0.9, evaluation_metric="accuracy"  # Hypothetical baseline
    )
    adaptive_opt = AdaptiveOptimizer(targeted_opt)

    # Define different search spaces
    search_spaces = {
        "small_discrete": {"n_estimators": [50, 100, 150], "max_depth": [5, 10, 15]},
        "large_mixed": {
            "n_estimators": (50, 300),
        ...
        "continuous_only": {"C": (0.01, 100.0), "gamma": (0.0001, 1.0)},
    }
