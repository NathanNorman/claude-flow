...
export const debugTestCase = {
  ...
  validateOutput: (output: string): boolean => {
    const debugConcepts = ['memory', 'leak', 'garbage collection', 'reference', 'event listener'];

    const hasDiagnostics = debugConcepts.some((concept) => output.toLowerCase().includes(concept));

    const hasSolution =
      output.includes('fix') || output.includes('solution') || output.includes('resolved');
    const hasCode =
      output.includes('```') || output.includes('function') || output.includes('const');
