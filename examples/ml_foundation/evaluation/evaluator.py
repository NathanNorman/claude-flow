...
import numpy as np
from ..models.base import MLESTARModel
...
class EvalConfig:
    ...
    regression_metrics: List[str] = field(
        default_factory=lambda: ["mse", "rmse", "mae", "r2", "explained_variance"]
    )
...
class EvaluationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "residuals": (
                self.residuals.tolist() if self.residuals is not None else None
            ),
...
class MetricsCalculator:
    ...
    def clustering_metrics(
        X: np.ndarray, labels: np.ndarray, true_labels: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
...
class CrossValidator:
    ...
    def cross_validate(
        self, model: MLESTARModel, X: np.ndarray, y: np.ndarray
    ) -> Dict[str, List[float]]:
...
class ReportGenerator:
    ...
    def save_report(self, evaluation_result: EvaluationResult, filepath: str) -> bool:
...
class ModelComparator:
    ...
    def compare_models(self, metric: str = "accuracy") -> Dict[str, Any]:
        ...
        comparison["rankings"] = [
            {"rank": i + 1, **model} for i, model in enumerate(model_scores)
        ]
    ...
    def statistical_comparison(self, metric: str = "accuracy") -> Dict[str, Any]:
        ...
        if len(cv_scores) < 2:
            return {"error": "Need CV scores for at least 2 models"}
        ...
        return {
            ...
            "summary": {
                ...
                "significant_differences": sum(
                    1 for comp in comparisons.values() if comp["significant"]
                ),
            },
        }
...
class ModelEvaluator:
    ...
    def evaluate_model(
        ...
        try:
            ...
            if task_type == "regression":
                metrics = self.metrics_calculator.regression_metrics(
                    y_test, predictions
                )
    ...
    def batch_evaluate(
        self, models: List[MLESTARModel], X_test: np.ndarray, y_test: np.ndarray
    ...
    def save_evaluation_results(
        self, results: List[EvaluationResult], directory: str = "./evaluation_results"
    ):
