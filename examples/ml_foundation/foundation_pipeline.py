...
class FoundationPipeline:
    ...
    def __init__(
        self, task_type: str = "classification", config: Optional[Dict] = None
    ):
        """
    ...
    def _get_default_config(self) -> Dict:
        ...
        base_config = {
            ...
            "validation": {"test_size": 0.2, "cv_folds": 5, "random_state": 42},
            "output": {
                "save_pipeline": True,
                "save_insights": True,
        ...
        if self.task_type == "classification":
            ...
            insights["basic_info"]["target_info"] = {
                ...
                "imbalance_ratio": float(
                    max(target_counts.values()) / min(target_counts.values())
                ),
            }
    ...
    def create_preprocessor(self, X: pd.DataFrame) -> ColumnTransformer:
        ...
        logger.info(
            f"Preprocessor created with {len(numeric_features)} numeric and "
        ...
        if self.task_type == "classification":
            ...
            models = {
                "linear_regression": LinearRegression(
                    **self.config["models"]["linear_regression"]
                ),
    ...
    def build_foundation(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        if self.config["preprocessing"]["feature_selection"]:
            ...
            if self.config["preprocessing"]["n_features"] == "auto":
                ...
                n_features = min(
                    self.config["preprocessing"]["n_features"],
        ...
        if self.task_type == "classification":
            best_model = max(
                performance_results.items(), key=lambda x: x[1]["f1_score"]
            )[0]
        else:
            best_model = max(
                performance_results.items(), key=lambda x: x[1]["r2_score"]
            )[0]

        # Step 8: Create final pipeline
        final_pipeline = Pipeline(
            [("preprocessor", preprocessor), ("model", baseline_models[best_model])]
        )
    ...
    def load_pipeline(self, path: str):
        ...
        if (path / "final_pipeline.pkl").exists():
            self.pipeline_components["final_pipeline"] = joblib.load(
...
def create_sample_dataset():
    ...
    X_df["category_B"] = np.random.choice(
        ["alpha", "beta", "gamma", "delta"], size=1000
    )
