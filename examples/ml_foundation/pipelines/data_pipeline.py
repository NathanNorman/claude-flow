...
class CacheManager:
    ...
    def _get_cache_key(
        self, data_config: Dict[str, Any], transform_params: Dict[str, Any]
    ) -> str:
...
class DataPipeline:
    ...
    def __init__(self, config: DataConfig):
        ...
        self.cache_manager = (
            CacheManager(config.cache_directory, config.cache_ttl)
    ...
    def add_minmax_scaler(
        self, feature_range: Tuple[float, float] = (0, 1)
    ) -> "DataPipeline":
    ...
    def split_data(
        ...
        if y_temp is not None:
            ...
            X_train, X_val = train_test_split(
                X_temp, test_size=val_size, random_state=self.config.random_state
            )
