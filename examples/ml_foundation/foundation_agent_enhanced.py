...
class EnhancedFoundationAgent:
    ...
    def __init__(
        ...
        self.session_id = (
            session_id or f"foundation-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )
    ...
    def _get_advanced_config(self) -> Dict:
        ...
        return {
            ...
            "validation": {"test_size": 0.2, "stratify": True, "random_state": 42},
            "output": {
                "save_models": True,
                "save_reports": True,
    ...
    def run(
        ...
        try:
            ...
            X_processed, preprocessing_pipeline = self._create_advanced_preprocessing(
                X, y
            )
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                ...
                stratify=(
                    ...
                    and self.config["validation"]["stratify"]
                    else None
                ),
            )

            # Step 7: Build baseline models
            baseline_results = self._build_baseline_models(
            ...
            if self.config["modeling"]["hyperparameter_tuning"]:
                best_model_info = self._optimize_hyperparameters(
                    X_train, y_train, X_test, y_test
                )
    ...
    def _analyze_missing_patterns(self, X: pd.DataFrame) -> Dict:
        ...
        patterns = {"random": True, "columns_always_missing_together": []}
    ...
    def _create_advanced_preprocessing(
        ...
        categorical_transformer = self._get_categorical_transformer(
            X[categorical_features] if categorical_features else pd.DataFrame()
        )
        ...
        if self.config["preprocessing"]["numeric_imputation"] == "knn":
            ...
            steps.append(
                (
                    "imputer",
                    SimpleImputer(
                        strategy=self.config["preprocessing"]["numeric_imputation"]
                    ),
                )
            )
    ...
    def _select_features(self, X: np.ndarray, y: pd.Series) -> Tuple[np.ndarray, Any]:
        ...
        if self.config["feature_selection"]["method"] == "auto":
            ...
            if self.config["feature_selection"]["n_features"] == "auto":
                ...
                n_features = min(
                    self.config["feature_selection"]["n_features"], X.shape[1]
                )
    ...
    def _build_baseline_models(self, X_train, y_train, X_test, y_test) -> Dict:
        ...
        models = self.model_builder.get_baseline_models(
            self.config["modeling"]["algorithms"]
        )
    ...
    def _optimize_hyperparameters(self, X_train, y_train, X_test, y_test) -> Dict:
        ...
        logger.info(
            f"Best optimized model: {best_model_info['name']} with score: {best_score:.4f}"
        )
    ...
    def _select_best_baseline(self) -> Any:
        ...
        if self.task_type == "classification":
            best_model_name = max(
                self.performance_metrics.items(), key=lambda x: x[1].get("f1_score", 0)
            )[0]
        else:
            best_model_name = max(
                self.performance_metrics.items(), key=lambda x: x[1].get("r2_score", 0)
            )[0]
    ...
    def _generate_comprehensive_report(
        self, X, y, X_test, y_test, pipeline, start_time
    ) -> Dict:
    ...
    def _get_preprocessing_summary(self) -> List[str]:
        ...
        if self.preprocessing_pipeline:
            for (
                name,
                transformer,
            ) in self.preprocessing_pipeline.named_transformers_.items():
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.data_insights.get("missing_analysis", {}).get("total_missing", 0) > 0:
            recommendations.append(
                "Consider advanced imputation methods for missing data"
            )

        if self.data_insights.get("outlier_analysis"):
            recommendations.append(
                "Outliers detected - consider robust scaling or outlier removal"
            )
        ...
        if best_score < 0.8:
            recommendations.append(
                "Model performance is moderate - consider feature engineering or ensemble methods"
            )

        # Based on task type
        if (
            self.task_type == "classification"
            and self.data_insights.get("target_analysis", {}).get("imbalance_ratio", 1)
            > 3
        ):
            recommendations.append(
                "Use SMOTE or adjust class weights for imbalanced data"
            )
    ...
    def _coordinate_handoff(self, results: Dict):
        ...
        try:
            ...
            handoff_data = {
                ...
                "pipeline_path": str(
                    Path(self.config["output"]["output_dir"]) / "final_pipeline.pkl"
                ),
    ...
    def _create_demo_dataset(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        X_df["category_A"] = np.random.choice(
            ["low", "medium", "high"], size=2000, p=[0.5, 0.3, 0.2]
        )
        X_df["category_B"] = np.random.choice(
            ["alpha", "beta", "gamma", "delta"], size=2000
        )
...
class FeatureEngineer:
    ...
    def engineer_features(
        self, X: np.ndarray, y: pd.Series, task_type: str
    ) -> np.ndarray:
        ...
        if self.task_type == "classification":
            if algorithm_set in ["auto", "basic"]:
                ...
                return {
                    ...
                    "mlp": MLPClassifier(
                        hidden_layers=(100, 50), max_iter=1000, random_state=42
                    ),
