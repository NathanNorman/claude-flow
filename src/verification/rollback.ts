  ...
  private async captureFiles(): Promise<FileSnapshot[]> {
    try {
      ...
      for (const path of criticalPaths) {
        try {
          const [content, stats] = await Promise.all([fs.readFile(path, 'utf-8'), fs.stat(path)]);
...
export class RollbackTrigger extends EventEmitter {
  ...
  private async getMemoryInfo() {
    try {
      ...
      const memTotal =
        parseInt(lines.find((l) => l.startsWith('MemTotal:'))?.split(/\s+/)[1] || '0') * 1024;
      const memAvailable =
        parseInt(lines.find((l) => l.startsWith('MemAvailable:'))?.split(/\s+/)[1] || '0') * 1024;
  ...
  private evaluateThresholds(metrics: SystemMetrics): string[] {
    ...
    if (metrics.performance.memory.percentage > this.config.thresholds.memoryUsage) {
      violations.push(
        `Memory usage: ${metrics.performance.memory.percentage.toFixed(1)}% > ${this.config.thresholds.memoryUsage}%`,
      );
    }

    if (metrics.performance.cpu.usage > this.config.thresholds.cpuUsage) {
      violations.push(
        `CPU usage: ${metrics.performance.cpu.usage.toFixed(1)}% > ${this.config.thresholds.cpuUsage}%`,
      );
    }

    const diskFreePercentage =
      (metrics.performance.disk.free / metrics.performance.disk.total) * 100;
    if (diskFreePercentage < this.config.thresholds.diskSpace) {
      violations.push(
        `Disk space: ${diskFreePercentage.toFixed(1)}% free < ${this.config.thresholds.diskSpace}%`,
      );
    }

    if (metrics.errors.rate > this.config.thresholds.errorRate) {
      violations.push(
        `Error rate: ${metrics.errors.rate}/min > ${this.config.thresholds.errorRate}/min`,
      );
    }
...
export class AutomatedRecovery extends EventEmitter {
  ...
  private async clearApplicationCaches(): Promise<void> {
    try {
      // Clear memory caches
      const memoryPaths = ['./memory/cache', './swarm-memory/cache', './temp'];
...
export class RollbackHistory extends EventEmitter {
  ...
  private async loadExistingHistory(): Promise<void> {
    try {
      ...
      const historyFiles = files.filter(
        (f) => f.endsWith('.history.json') || f.endsWith('.history.json.gz'),
      );
  ...
  public getSuccessRate(timeframe: number = 24 * 60 * 60 * 1000): number {
    ...
    const recentEntries = Array.from(this.history.values()).filter(
  ...
  public getStatistics(): {
    ...
    const rollbacksByTrigger = entries.reduce(
      (acc, entry) => {
...
export class GitRollbackManager extends EventEmitter {
   ...
   */
  public async rollbackFiles(files: string[], commitHash: string): Promise<boolean> {
    try {
      if (this.safetyChecks) {
        await this.validateCommit(commitHash);
   ...
   */
  public async listRollbackPoints(): Promise<
    Array<{
      hash: string;
      date: Date;
      message: string;
...
export class RollbackManager extends EventEmitter {
   ...
   */
  public async createCheckpoint(description: string, tags: string[] = []): Promise<string> {
    try {
  ...
  private async handleAutomaticRollback(metrics: SystemMetrics, reason: string): Promise<void> {
    try {
  ...
  public getRollbackTrigger(): RollbackTrigger {
    return this.rollbackTrigger;
  }
  public getAutomatedRecovery(): AutomatedRecovery {
    return this.automatedRecovery;
  }
  public getRollbackHistory(): RollbackHistory {
    return this.rollbackHistory;
  }
  public getGitRollbackManager(): GitRollbackManager {
