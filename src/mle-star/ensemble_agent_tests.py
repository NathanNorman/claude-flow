...
def test_classification_ensembles():
    ...
    for i, model in enumerate(base_models):
        ...
        print(
            f"{model.__class__.__name__}: {accuracy:.4f} (train time: {train_time:.2f}s)"
        )
    ...
    for strategy in strategies:
        try:
            ...
            print(
                f"{strategy}: {accuracy:.4f} (improvement: {improvement:+.4f}, "
...
def test_regression_ensembles():
    ...
    for i, model in enumerate(base_models):
        ...
        print(
            f"{model.__class__.__name__}: R²={r2:.4f}, MSE={mse:.2f} (train time: {train_time:.2f}s)"
        )
    ...
    for strategy in strategies:
        try:
            ...
            print(
                f"{strategy}: R²={r2:.4f}, MSE={mse:.2f} (improvement: {improvement:+.4f}, "
                f"train: {train_time:.2f}s, pred: {pred_time:.3f}s)"
            )
...
def test_advanced_ensembles():
    ...
    for i in range(3):
        ...
        X_val, _, y_val, _ = train_test_split(
            X_test, y_test, test_size=0.5, random_state=i
        )
        metrics = monitor.track_performance(
            ensemble, X_val, y_val, timestamp=f"eval_{i}"
        )
...
def main():
    ...
    for strategy, results in classification_results.items():
        print(
            f"  {strategy}: Accuracy={results['accuracy']:.4f}, "
