...
class AttentionBasedEnsemble(BaseEstimator):
    ...
    def _build_attention_network(self, input_dim: int, n_models: int):
        ...
        class AttentionNet(nn.Module):
            ...
            def forward(self, x, model_outputs):
                ...
                scores = (
                    torch.matmul(Q.unsqueeze(1), K.unsqueeze(2)).squeeze() / self.scale
                )
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            weighted_outputs, attention_weights = self.attention_network(
                X_tensor, outputs_tensor
            )
            ensemble_pred = weighted_outputs.sum(dim=1)
...
class AdaptiveEnsembleSelector:
    ...
    def analyze_data_characteristics(self, X, y) -> Dict[str, float]:
        ...
        chars = {
            ...
            "class_balance": (
                self._compute_class_balance(y) if len(np.unique(y)) < 20 else 1.0
            ),
        }
    ...
    def create_adaptive_ensemble(
        self, X, y, models: List[BaseEstimator]
    ) -> BaseEstimator:
...
class EnsembleMonitor:
    ...
    def analyze_model_contributions(self, ensemble, X_sample):
        ...
        if hasattr(ensemble, "weights"):
            ...
            contributions = {
                f"model_{i}": weight for i, weight in enumerate(ensemble.weights)
            }
    ...
    def _generate_recommendations(self):
        ...
        if self.performance_history:
            ...
            if "accuracy" in latest and latest["accuracy"] < 0.8:
                ...
                recommendations.append(
                    "Consider feature engineering or different model architectures"
                )
        ...
        if self.model_contributions:
            ...
            if max_contrib > 0.7:
                recommendations.append(
                    "Single model dominates - consider removing weak models"
                )
...
if __name__ == "__main__":
    ...
    print(
        f"Model contributions: {monitor.analyze_model_contributions(ensemble, X_test[:10])}"
    )
