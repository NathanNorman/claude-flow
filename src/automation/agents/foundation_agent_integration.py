...
class FoundationAgentIntegration:
    ...
    def _store_memory(self, key: str, value: Any):
        ...
        cmd = ["npx", "claude-flow@alpha", "memory", "store", key, json.dumps(value)]
    ...
    def check_research_findings(self) -> Optional[Dict]:
        ...
        search_results = self._query_memory(
            f"workflow/{self.execution_id}/web_search_phase"
        )
    ...
    def process_workflow_step(self, step_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Store result
            self._store_memory(f"agent/foundation_agent/{step_type}_result", result)

            # Run post-task hook
            self._run_hook(
                "post-task", task_id=self.agent_id, analyze_performance="true"
            )
    ...
    def share_findings(self):
        ...
        if self.builder and self.builder.baseline_results:
            ...
            self._store_memory(
                f"workflow/{self.execution_id}/foundation_findings", findings
            )
...
def main():
    ...
    if research:
        print("✓ Loaded research findings from search agent")
